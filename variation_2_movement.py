...
def move_straight(robot, unit):
    robot.drive_straight(distance_mm(abs(unit)*100),speed_mmps(200)).wait_for_completed()
    return
...
def move(robot, current_x, current_y, x , y):
    ...
    robot.say_text(f"I will cover {abs(x-current_x)*10} cm horizontally {x_direction} and {abs(y-current_y)*10} cm vertically {y_direction}").wait_for_completed()
